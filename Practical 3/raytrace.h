...
bool inHardShaddow(RayTriangleIntersection rTI, vec3 lightSource, vector<ModelTriangle> triangles);
  ...
  for(int i = 0; i < WIDTH - 1; i++){
    for(int j = 0; j < HEIGHT - 1; j++){
      ...
      if(intersection.distanceFromCamera != std::numeric_limits<float>::infinity()){
        ...
        if(inHardShaddow(intersection, lightSource, triangles)){
